...
function Mod:CHALLENGE_MODE_RESET()
	...
	MythicPlusDrop_Data.state.splits = nil
	MythicPlusDrop_Data.state.splitNames = nil
...
function Mod:CHALLENGE_MODE_COMPLETED()
	...
	if Addon.Config.recordSplits then
		if not MythicPlusDrop_Data.splits[mapID] then MythicPlusDrop_Data.splits[mapID] = {} end
		table.insert(MythicPlusDrop_Data.splits[mapID], splits)
	end
end
...
function Mod:SCENARIO_UPDATE()
	...
	if scenarioType == LE_SCENARIO_TYPE_CHALLENGE_MODE then
		...
		if not Mod.splits and numCriteria > 0 then
			Mod.splits = {}
			MythicPlusDrop_Data.state.splits = Mod.splits
			Mod.splitNames = {}
			MythicPlusDrop_Data.state.splitNames = Mod.splitNames
			MythicPlusDrop_Data.state.mapID = mapID
...
function Mod:Startup()
	if not MythicPlusDrop_Data then MythicPlusDrop_Data = {} end
	if not MythicPlusDrop_Data.splits then MythicPlusDrop_Data.splits = {} end
	if not MythicPlusDrop_Data.state then MythicPlusDrop_Data.state = {} end

	local mapID = C_ChallengeMode.GetActiveChallengeMapID()
	if select(10, C_Scenario.GetInfo()) == LE_SCENARIO_TYPE_CHALLENGE_MODE and mapID and mapID == MythicPlusDrop_Data.state.mapID then
		Mod.splits = MythicPlusDrop_Data.state.splits
		Mod.splitNames = MythicPlusDrop_Data.state.splitNames
	else
		MythicPlusDrop_Data.state.mapID = nil
		MythicPlusDrop_Data.state.splits = nil
		MythicPlusDrop_Data.state.splitNames = nil
